...
func create_faction(name: String, description: String) -> Faction:
    """Create a new faction"""
    var faction = Faction.new(name)
    faction.description = description
    factions[name] = faction
    character_reputation[name] = 0  # Start at neutral
    return faction

func setup_harper_faction():
    """Setup Harper faction details"""
    var harpers = factions["Harpers"]
...
func setup_zhentarim_faction():
    ...
    var zhentarim = factions["Zhentarim"]
...
func setup_emerald_enclave_faction():
    ...
    var enclave = factions["Emerald Enclave"]
...
func setup_lords_alliance_faction():
    ...
    var alliance = factions["Lord's Alliance"]
...
func setup_order_gauntlet_faction():
    ...
    var order = factions["Order of the Gauntlet"]
...
func change_reputation(faction: String, amount: int, reason: String = ""):
...
func get_faction_benefits(faction: String) -> Array[String]:
    ...
    if reputation_level >= ReputationLevel.FRIENDLY and "friendly" in faction_data.benefits:
        benefits.append_array(faction_data.benefits["friendly"])

    if reputation_level >= ReputationLevel.HONORED and "honored" in faction_data.benefits:
        benefits.append_array(faction_data.benefits["honored"])

    if reputation_level >= ReputationLevel.EXALTED and "exalted" in faction_data.benefits:
        benefits.append_array(faction_data.benefits["exalted"])
