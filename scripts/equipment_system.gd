...
func setup_equipment_sets():
    ...
    equipment_sets = {
        "Leather Armor Set": {
            "pieces": ["Leather Armor", "Leather Boots", "Leather Gloves"],
            "bonuses": {
                "stealth": 2,
                "movement_speed": 5
            }
        ...
        "Chain Mail Set": {
            "pieces": ["Chain Mail", "Chain Boots", "Chain Gauntlets"],
            "bonuses": {
                "armor_class": 1,
        ...
        "Plate Armor Set": {
            "pieces": ["Plate Armor", "Plate Boots", "Plate Gauntlets", "Plate Helmet"],
            "bonuses": {
                "armor_class": 2,
        ...
        "Wizard's Set": {
            "pieces": ["Wizard's Robe", "Wizard's Hat", "Wizard's Staff"],
            "bonuses": {
                "spell_save_dc": 1,
...
func can_attune_item(character: Character, item: EquipmentResource) -> bool:
...
func apply_item_stats(character: Character, item: EquipmentResource, equip: bool):
    ...
    if item.has_method("get_ability_bonuses"):
        ...
        for ability in bonuses:
            var bonus = bonuses[ability] * multiplier
    ...
    if item.has_method("get_skill_bonuses"):
        ...
        for skill in bonuses:
            var bonus = bonuses[skill] * multiplier
...
func apply_set_bonuses(character: Character, set_data: Dictionary, apply: bool):
    ...
    for bonus_type in bonuses:
        var bonus_value = bonuses[bonus_type] * multiplier
...
func get_armor_class_bonus(character: Character) -> int:
...
func get_weapon_bonus(character: Character) -> int:
...
func get_damage_bonus(character: Character) -> int:
...
func get_equipment_summary(character: Character) -> Dictionary:
    """Get a summary of all equipment effects"""
    return {
        "armor_class_bonus": get_armor_class_bonus(character),
        "weapon_bonus": get_weapon_bonus(character),
        "damage_bonus": get_damage_bonus(character),
