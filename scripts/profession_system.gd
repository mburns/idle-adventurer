...
func setup_professions():
	...
	create_artisan_professions()
	create_merchant_professions()
	create_scholar_professions()
	create_guard_professions()
	create_priest_professions()
	create_entertainer_professions()
	create_healer_professions()
	create_informant_professions()
	create_administrator_professions()
	create_servant_professions()

func create_artisan_professions():
	"""Create artisan professions"""
	var blacksmith = Profession.new("blacksmith", "Blacksmith", ProfessionType.ARTISAN)
	blacksmith.description = "Craft weapons, armor, and metal tools"
	blacksmith.requirements = {"strength": 13, "gold": 100}
	blacksmith.daily_income = 15
	blacksmith.reputation_gain = 2
	blacksmith.skill_requirements = ["smith_tools"]
	blacksmith.tool_requirements = ["smith_tools", "forge"]
	blacksmith.location = "smithy"
	blacksmith.schedule = {"morning": "work", "afternoon": "work", "evening": "rest"}
	blacksmith.advancement_requirements = {"level": 5, "reputation": 50}
	blacksmith.benefits = {"weapon_discount": 0.2, "armor_discount": 0.2}
	professions["blacksmith"] = blacksmith

	var jeweler = Profession.new("jeweler", "Jeweler", ProfessionType.ARTISAN)
	jeweler.description = "Craft jewelry and work with precious stones"
	jeweler.requirements = {"dexterity": 14, "gold": 200}
	jeweler.daily_income = 25
	jeweler.reputation_gain = 3
	jeweler.skill_requirements = ["jeweler_tools"]
	jeweler.tool_requirements = ["jeweler_tools", "workbench"]
	jeweler.location = "artisan_quarter"
	jeweler.schedule = {"morning": "work", "afternoon": "work", "evening": "rest"}
	jeweler.advancement_requirements = {"level": 6, "reputation": 60}
	jeweler.benefits = {"jewelry_discount": 0.3, "gem_appraisal": true}
...
func calculate_profession_level(experience: int) -> int:
	...
	return min(20, (experience / 100) + 1)
