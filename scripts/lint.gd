...
func check_naming_conventions(line: String, file_path: String, line_number: int):
    ...
    if class_result:
        var class_type = class_result.get_string(1)
        if not is_pascal_case(class_type):
            lint_errors.append("%s:%d: Class '%s' should use PascalCase" % [file_path, line_number, class_type])
